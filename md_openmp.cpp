...
void compute(int np, int nd, double pos[], double vel[], double mass,
             double f[], double *pot, double *kin, int curr_step);
...
//
{
  ...
  double mass = 1.0;
  int nd = 3;
  int np = 13; // np = NUMBER OF PARTICLES
  double *pos;
  ...
  cout << "  Numeric Limit of a double = " << std::numeric_limits<double>::min()
  ...
  compute(np, nd, pos, vel, mass, force, &potential, &kinetic, step);
  ...
  cout << "                Energy P        Energy K       Relative Energy "
          "Error\n";
  cout << "\n";

  step_print = 0;
  step_print_index = 0;
  step_print_num = step_num / 2; // sets the frequency of printing information.
                                 // step_num / 2 will print every other step

  step = 0;
  cout << "  " << setw(8) << step << "  " << setw(14) << potential << "  "
       << setw(14) << kinetic << "  " << setw(14)
  ...
  //
  // open file to append the particle positions to a file in xyz format
  //
  fstream outfile("output.txt", std::fstream::out);
  ...
  for (step = 1; step <= step_num; step++) {
    ...
    if (step == step_print) {
      ...
      //
      // append the particle positions to a file in xyz format
      //
      outfile << endl;
      for (int i = 0; i < np; i++) {
        outfile << setprecision(10);
        outfile << setw(10) << left << i;
        outfile << setw(20) << left << pos[0 + i * nd];
        outfile << setw(20) << left << pos[1 + i * nd];
        outfile << setw(20) << left << pos[2 + i * nd] << endl;
      }
    }
...
void compute(int np, int nd, double pos[], double vel[], double mass,
             double f[], double *pot, double *kin, int current_step)
  ...
  for (k = 0; k < np; k++) {
    ...
    for (j = 0; j < np; j++) {
      if (k != j) {
        ...
        // PRINT THE POSITIONS FOR THE CURRENT STEP
        /* 	if (current_step == 2) {
...
//
{
  ...
  //
  // print the particle positions to a new file in xyz format
  //
  fstream outfile("output.txt", std::fstream::out);
  outfile << endl;
  for (int i = 0; i < np; i++) {
